...
export default function Students() {
  ...
  const [approveOnSave, setApproveOnSave] = useState(false); // NEW: Track if we are approving
  ...
  // UPDATED: Now accepts an 'approve' boolean to link approval with scheduling
  const openScheduleModal = (student: any, approve = false) => {
    setSelectedStudent(student);
    setTempStartDate(student.start_date || '');
    setTempEndDate(student.end_date || '');
    setApproveOnSave(approve);
    setShowScheduleModal(true);
  };

  // UPDATED: Saves dates AND approves the student in one go if approveOnSave is true
  const handleSaveSchedule = async () => {
    ...
    // If the admin clicked "Approve", we also update the status to 'approved'
    if (approveOnSave) {
    ...
    if (error) {
      ...
      const successMsg = approveOnSave 
        ? `Student approved and schedule set successfully!` 
        : 'Schedule saved successfully!';
        
      setNotification({ message: successMsg, type: 'success' });
      
      const newStatus = approveOnSave ? 'approved' : selectedStudent.status;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 md:pl-64 transition-colors duration-500">
                        ...
                        <td className="p-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            {student.status === 'archived' ? (
                              ...
                              <>
                                <button onClick={() => openScheduleModal(student, false)} className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-200 transition" title="Set Schedule">
                                ...
                                {/* CHANGED: Approve button now opens the Schedule Modal directly! */}
                                {student.status === 'pending' && (
                                  <button onClick={() => openScheduleModal(student, true)} className="p-2 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-200 transition" title="Approve & Set Schedule">
      ...
      {/* Schedule Modal (Now handles both Scheduling and Approving) */}
      <AnimatePresence>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
                <h3 className="font-bold text-lg text-gray-800 dark:text-white flex items-center gap-2">
                  {approveOnSave ? <><CheckCircle className="w-5 h-5 text-green-600" /> Approve & Set Schedule</> : <><Calendar className="w-5 h-5 text-purple-600" /> Set Training Schedule</>}
                </h3>
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {approveOnSave ? 'Approving and setting schedule for' : 'Setting schedule for'} 
                  <span className="font-bold text-purple-600 dark:text-purple-400"> {selectedStudent.full_name}</span>
                </p>
                <div>
                ...
                <button onClick={handleSaveSchedule} disabled={isSaving || !tempStartDate || !tempEndDate} className="w-full btn-primary flex items-center justify-center gap-2 disabled:opacity-70">
                  {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : (approveOnSave ? <CheckCircle className="w-5 h-5" /> : <Calendar className="w-5 h-5" />)}
                  {isSaving ? 'Processing...' : (approveOnSave ? 'Approve & Start Countdown' : 'Save Schedule')}
                </button>
              </div>
            </motion.div>
          </motion.div>
